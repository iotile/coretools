...
from iotile.core.hw.virtual import VirtualIOTileDevice, EmulatedDevice
...
def main():
    ...
    parser.add_argument('-s', '--state', help="Load a given state into the device before starting to serve it.  This only works with EmulatedDevices")
    parser.add_argument('-t', '--track', help="Track all changes to the device's state.  Only works with emulated devices.")
    args, _rest = list_parser.parse_known_args()
    ...
    try:
        ...
        if (args.state or args.track) and not isinstance(device, EmulatedDevice):
            print("ERROR: you can only use --state or --track if you are serving an emulated device.")
            return 1
