...
class BasicAuthProvider(AuthProvider):
    """Basic default authentication provider that can sign with a UUID only
    """

    def sign(self, device_id, sig_method, data):
        """Sign a buffer of data on behalf of a device

        This routine only supports hash only signatures and unsecure signing
        just using the device's uuid.
        ...
        elif method_name == 'hmac_sha256_uuid':
            device_bytes = struct.pack("<L", device_id)

            hmac_calc = hmac.new(device_bytes, data, hashlib.sha256)
