...
def generate_doxygen_file(output_path, iotile):
	...
	input_template = os.path.join(resource_filename(Requirement.parse("iotilebuild"), "iotilebuild/config"), 'templates', 'doxygen.txt')
...
def doxygen_source_path():
	return os.path.abspath(os.path.join(resource_filename(Requirement.parse("iotilebuild"), "iotilebuild/config"), 'templates', 'doxygen.txt'))
