"""Entrypoint for the iotile-emulate script that can serve an emulated iotile device."""
...
from iotile.core.hw.virtual import VirtualIOTileInterface
from iotile.emulate import EmulatedDevice

logger = None

class EarlyExit(Exception):
    """An exception to exit the script with the given exit code."""
...
def build_parsers():
    """Build the argument parsers for this script."""
    ...
    parser = argparse.ArgumentParser(description="Serve acess to an emulated IOTile device using a virtual IOTile interface")
    ...
    parser.add_argument('-d', '--dump', help="Dump the device's state when we exit the program.")
    parser.add_argument('-t', '--track', help="Track all changes to the device's state.")
    parser.add_argument('-v', '--verbose', action="count", default=0, help="Increase logging level (goes error, warn, info, debug)")

    return list_parser, parser


def list_plugins():
    """List all installed interfaces and device."""

    reg = ComponentRegistry()
    print("Installed Virtual Interfaces:")
    for name, _iface in reg.load_extensions('iotile.virtual_interface',
                                            class_filter=VirtualIOTileInterface):
        print('- {}'.format(name))

    print("\nInstalled Device Emulators:")
    for name, dev in reg.load_extensions('iotile.virtual_device', class_filter=EmulatedDevice,
...
def configure_logging(verbose):
    """Setup our logger according to the verbosity level."""
...
def main(argv=None):
    ...
    list_parser, parser = build_parsers()
    args, _ = list_parser.parse_known_args(argv)
    ...
    try:
        ...
        if args.state is not None:
            logger.info("Loading device state from file %s", args.state)
            device.load_state(args.state)

        if args.scenario is not None:
            logger.info("Loading scenario from file %s", args.scenario)
        ...
        if args.track is not None:
            logger.info("Tracking all state changes to device")
            device.state_history.enable()

        iface.start(device)
        started = True

        logger.info("Starting to serve emulated device")
        ...
        logger.info("Break received, cleanly exiting...")
    except EarlyExit as exc:
        if exc.code != 0:
            logger.exception("Exiting with error code %d due to exception", exc.code)

        return_value = exc.code
    finally:
        if args.dump is not None and device is not None:
            logger.info("Dumping final device state to %s", args.dump)
        ...
        if args.track is not None and device is not None:
            logger.info("Saving state history to file %s", args.track)
...
def instantiate_device(virtual_dev, config):
    ...
    Returns:
        EmulatedDevice: The instantiated subclass of EmulatedDevice
    """
    conf = {}
    if 'device' in config:
        conf = config['device']
    ...
    try:
        ...
        if virtual_dev.endswith('.py'):
            _, dev = reg.load_extension(virtual_dev, class_filter=EmulatedDevice,
                                        unique=True)
        else:
            _, dev = reg.load_extensions('iotile.virtual_device', name_filter=virtual_dev,
                                         class_filter=EmulatedDevice,
                                         product_name="virtual_device", unique=True)

        return dev(conf)
    except ArgumentError as err:
        logger.error("ERROR: Could not load virtual device (%s): %s", virtual_dev, err.msg)
        raise EarlyExit(1) from err
...
def instantiate_interface(virtual_iface, config):
    ...
    try:
        ...
        if virtual_iface.endswith('.py'):
            _, iface = reg.load_extension(virtual_iface,
                                          class_filter=VirtualIOTileInterface, unique=True)
        else:
            _, iface = reg.load_extensions('iotile.virtual_interface', name_filter=virtual_iface,
                                           class_filter=VirtualIOTileInterface, unique=True)

        return iface(conf)
    except ArgumentError as err:
        logger.error("ERROR: Could not load virtual interface (%s): %s", virtual_iface, err.msg)
        raise EarlyExit(2) from err
