        ...
        if read_length is None:
            ...
            if region == 'ram':
                if read_length > device_info.ram_size:
                    raise ArgumentError("Invalid data length to read from RAM.", ram_size=device_info.ram_size)
                if read_start_addr > device_info.ram_size:
                    raise ArgumentError("Invalid start address to read from RAM.", ram_size=device_info.ram_size)
                memory = self._read_memory(read_start_addr + device_info.ram_start, read_length)

            elif region == 'flash':
                if read_length > device_info.flash_size:
                    raise ArgumentError("Invalid data length to read from flash.", flash_size=device_info.flash_size)
                if read_start_addr > device_info.flash_size:
                    raise ArgumentError("Invalid start address to read from flash.", flash_size=device_info.flash_size)
                ...
                if read_start_addr > 0:
                    ...
                    raise ArgumentError("Invalid start address to read from memory map.")
