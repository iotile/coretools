...
import serial
from iotile.core.utilities.async_tools import BackgroundEventLoop, SharedLoop, OperationManager
...
from iotile_transport_blelib.interface import (AbstractBLECentral, BLEScanDelegate, BLEAdvertisement,
                                               BLEPeripheral, errors, messages, BLECentralState, EmptyScanDelegate)
...
class _ConnectionData:
    def __init__(self, conn_string: str, handle: int,
                 #delegate: Optional[BLEPeripheralDelegate],
                 peripheral: Optional[BLED112Peripheral]):
...
class BLED112Central(AbstractBLECentral):
    ...
    def __init__(self, device: Union[serial.Serial, str], loop: BackgroundEventLoop = SharedLoop):
        ...
        self._operation_manager = OperationManager(loop=self._loop) #type: OperationManager[messages.BluetoothEvent]
        self._scanners = BLEScanManager(self._operation_manager)
    ...
    async def on_advertisement(self, packet: packets.BGAPIPacket):
        ...
        try:
            ...
            length = len(payload) - 11
            rssi, packet_type, sender, _, _, _length, data = struct.unpack("<bB6sBBB%ds" % length, payload)
