...
class WebSocketHandler:
    ...
    def on_message(self, message):
        ...
        if cmdcode == 'scan':
            ...
            resp = yield from self.manager.connect(cmd['uuid'])
            ...
            if self.connection is not None:
                resp = yield from self.manager.disconnect(self.connection)
            ...
            if self.connection is not None:
                resp = yield from self.manager.open_interface(self.connection, cmd['interface'])
                self.send_response(resp)
            else:
                self.send_error('Attempt to open IOTile interface when there was no connection')
        elif cmdcode == 'send_rpc':
            if self.connection is not None:
                resp = yield from self.manager.send_rpc(
                    self.connection,
                    cmd['rpc_address'],
                    cmd['rpc_feature'],
                    cmd['rpc_command'],
            ...
            if self.connection is not None:
                resp = yield from self.manager.send_script(
                    self.connection,
                    cmd['data'],
    ...
    def on_close(self):
        if self.connection is not None:
            yield from self.manager.disconnect(self.connection)
            self.connection = None
