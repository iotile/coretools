...
class ComponentRegistryResolver(DependencyResolver):
    ...
    def resolve(self, depinfo, destdir):
        ...
        if not os.path.exists(comp.output_folder):
            raise ExternalError("Component found in registry but has not been built", path=comp.folder,
                                name=comp.name, suggestion="Run iotile build on this component first")

        try:
            IOTile(comp.output_folder)
        except IOTileException:
            raise ExternalError("Component found in registry but its build/output folder is not valid",
                                path=comp.folder, name=comp.name, suggestion="Cleanly rebuild the component")
    ...
    def check(self, depinfo, deptile, depsettings):
        ...
        # If the component in the registry has a higher version or a newer release date, it should be updated
