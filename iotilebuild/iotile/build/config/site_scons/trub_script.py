...
def build_update_script(file_name, slot_assignments=None, os_info=None, use_safemode=False, sensor_graph=None, app_info=None):
...
def _build_reflash_script_action(target, source, env):
    ...
    if env['USE_SAFEMODE']:
        safemode_enable = SendRPCRecord(8,0x1006,int(True))
    ...
    if env['USE_SAFEMODE']:
        safemode_disable = SendRPCRecord(8,0x1006,int(False))
