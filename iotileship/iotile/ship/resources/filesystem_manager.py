...
RESOURCE_ARG_SCHEMA.add_required("res_path",StringVerifier("path to resource to work with. "
    "Specify file:// for a directory or sd:// for an unmounted disk partition"))


class FilesystemManagerResource(SharedResource):
    ARG_SCHEMA = RESOURCE_ARG_SCHEMA
    _allowed_types = ["file", "sd"]
    ...
    def __init__(self, args):
        ...
        if self._type == "sd" and sys.platform != "linux":
            raise ArgumentError("sd;// paths are only supported on Linux. \
    ...
    def open(self):
        if self._type == "sd":
            ...
            if self.root is None: 
                #Mount
                mount_cmd = ['udisksctl', 'mount', '--block-device', self._resource_loc, 
                                '--no-user-interaction']
            ...
            print("Mounted to: " + str(self.root))
    ...
    def close(self):
        if self._type == "sd" and self._unmount_in_close:
